                                ...
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <?php if(session()->has('deletedLogs')): ?>
                                        <?php foreach( $logs->where('deleted', '1')->get() as $log): ?>
                                            <tr class="odd gradeX">
                                                <td><?php echo e($users->where('id', $log->user_id)->first()->name == auth()->user()->name ? "You" : $users->where('id', $log->user_id)->first()->name); ?></td>
                                    ...
                                    <?php else: ?>
                                        <?php foreach( $logs->where(['deleted' => '0'])->get() as $log): ?>
                                            <?php if($log->user_id == auth()->user()->id or $users->find($log->user_id)->under_to == auth()->user()->id): ?>
                                                <tr class="odd gradeX">
                                                    <td><?php echo e($users->where('id', $log->user_id)->first()->name == auth()->user()->name ? "You" : $users->where('id', $log->user_id)->first()->name); ?></td>
                                                    <td><?php echo e($log->activity); ?></td>
